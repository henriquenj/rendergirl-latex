...
\item In the end it was used the \emph{ctypes} python module, capable
  of loading shared libraries if there is a C API. This way the C
...
\begin{frame}
  ...
  It was also tested against RenderGirl prior the implementation of
  BVH, where only one object is visible to the camera and the
  remaining are hidden

  On some scenes, we non-BVH version crashs the GPU driver, which
...
\begin{itemize}
\item Distribute render into \emph{passes} as a measure to prevent the crash on the version without BVH.
