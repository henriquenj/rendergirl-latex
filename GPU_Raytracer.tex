...
program and focuses on the rendering tasks. We developed a C++
...
different results on different setups. It cannot have complex data
...
hierarchically subdivided a scene so the queries for the objects
...
partioning structure suitable for stackless traversal on the GPU.
...
log operations. Most of the interactions with the raytracer occurs
...
a scene using RenderGirl data structures; it's also responsible by
subdividing the scene into BVH structures and make it suitable for
...
function which can takes several minutes to complete. The reasoning
behind this is that every host program will provided different
asynchronous APIs for its plugin system in a different manner (or not
...
their own. As a initial design decision to ease portability, the core
...
interpret the Y axis as the depth instead of height. We also make use
...
a implementation using convex hulls as bounding volumes for each level
of the tree.
...
thesis\cite{Thrane}. The main problem to be solved here is overcome
the lack of stack on GPU hardware, since tree traversal algorithms are
...
so we never transmit the tree to the OpenCL device and rather the
iteration itself. The traversal array already contains the nodes we
must intersect in the corret order. The BVH node on the traversal
array have three properties: the AABB that fits all the geometry of
that node and its children, the \emph{escape index} and a index that
...
figure~\ref{fig:bvh}. The object index is only valid on child nodes,
and it's flagged as -1 when on a middle node. The traversal is done in
a top-botton left to right order. The algorithm for traversal is
...
and it is the application responsability to make sure the kernel can
