...
\large
  ...
  renderer called RenderGirl suitable for running inside the OpenCL
  framework. We aim to take advantage of heterogeneous computing
  devices such as GPUs and many-core CPUs, focusing on paral-
  lelism. We implemented the traditional partitioning scheme called
  bounding volume hierarchies, where each scene is hierarchically
  subdivided into axis-aligned bounding boxes, so a ray may only need
  to traverse a subset of geometry by traversing the BVH and
  discarding objects it surely cannot hit, optimizing the rendering
  ...
  binary tree structure implementation ready for stackless traversal
  on GPUs. RenderGirl is split between two main portions: Core and
  ...
  interfaces provide communication with a given host program, seek-
  ing modularity. In this paper we describe our results and perfor-
  mance gains with our partitioning scheme.
